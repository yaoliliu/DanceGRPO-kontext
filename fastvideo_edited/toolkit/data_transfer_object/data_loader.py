...
from toolkit.dataloader_mixins import CaptionProcessingDTOMixin, ImageProcessingDTOMixin, LatentCachingFileItemDTOMixin, \
    ...
    UnconditionalFileItemDTOMixin, ClipImageFileItemDTOMixin, InpaintControlFileItemDTOMixin, ReferencedFileItemDTOMixin
...
class DataLoaderBatchDTO:
    def __init__(self, **kwargs):
        try:
            ...
            # Edited: 增加referenced_tensor
            self.referenced_tensor: Union[torch.Tensor, None] = None
            if any([x.referenced_tensor is not None for x in self.file_items]):
                # find one to use as a base
                base_referenced_tensor = None
                for x in self.file_items:
                    if x.referenced_tensor is not None:
                        base_referenced_tensor = x.referenced_tensor
                        break
                referenced_tensors = []
                for x in self.file_items:
                    if x.referenced_tensor is None:
                        referenced_tensors.append(torch.zeros_like(base_referenced_tensor))
                    else:
                        referenced_tensors.append(x.referenced_tensor)
                self.referenced_tensor = torch.cat([x.unsqueeze(0) for x in referenced_tensors])

...
class ImageProcessingDTOMixin:
    ...
    def load_and_process_image(
        ...
        if self.is_latent_cached:
            ...
            if self.has_referenced_image:
                self.load_referenced_image()
...
class ReferencedFileItemDTOMixin:
    def __init__(self: 'FileItemDTO', *args, **kwargs):
        if hasattr(super(), '__init__'):
            super().__init__(*args, **kwargs)
        self.has_referenced_image = False
        self.referenced_path: Union[str, List[str], None] = None
        self.referenced_tensor: Union[torch.Tensor, None] = None
        dataset_config: 'DatasetConfig' = kwargs.get('dataset_config', None)
        self.full_size_referenced_images = False
        if dataset_config.referenced_path is not None:
            # find the referenced image path
            referenced_path_list = dataset_config.referenced_path
            if not isinstance(referenced_path_list, list):
                referenced_path_list = [referenced_path_list]
            self.full_size_referenced_images = dataset_config.full_size_referenced_images
            # we are using referenced images
            img_path = kwargs.get('path', None)
            file_name_no_ext = os.path.splitext(os.path.basename(img_path))[0]

            found_referenced_images = []
            for referenced_path in referenced_path_list:
                for ext in img_ext_list:
                    if os.path.exists(os.path.join(referenced_path, file_name_no_ext + ext)):
                        found_referenced_images.append(os.path.join(referenced_path, file_name_no_ext + ext))
                        self.has_referenced_image = True
                        break
            self.referenced_path = found_referenced_images
            if len(self.referenced_path) == 0:
                self.referenced_path = None
            elif len(self.referenced_path) == 1:
                # only do one
                self.referenced_path = self.referenced_path[0]

    def load_referenced_image(self: 'FileItemDTO'):
        referenced_tensors = []
        referenced_path_list = self.referenced_path
        if not isinstance(self.referenced_path, list):
            referenced_path_list = [self.referenced_path]

        for referenced_path in referenced_path_list:
            try:
                img = Image.open(referenced_path).convert('RGB')
                img = exif_transpose(img)
            except Exception as e:
                print_acc(f"Error: {e}")
                print_acc(f"Error loading image: {referenced_path}")

            if not self.full_size_referenced_images:
                # we just scale them to 512x512:
                w, h = img.size
                img = img.resize((512, 512), Image.BICUBIC)
                ...
                if self.dataset_config.buckets:
                    ...
                    raise Exception("Referenced images not supported for non-bucket datasets")
            transform = transforms.Compose([
                transforms.ToTensor(),
            ])
            if self.aug_replay_spatial_transforms:
                tensor = self.augment_spatial_referenced(img, transform=transform)
            else:
                tensor = transform(img)
            referenced_tensors.append(tensor)

        if len(referenced_tensors) == 0:
            self.referenced_tensor = None
        elif len(referenced_tensors) == 1:
            self.referenced_tensor = referenced_tensors[0]
        else:
            self.referenced_tensor = torch.stack(referenced_tensors, dim=0)

    def cleanup_referenced(self: 'FileItemDTO'):
        self.referenced_tensor = None
    ...
    def cache_latents_all_latents(self: 'AiToolkitDataset'):
        ...
        with accelerator.main_process_first():
            ...
            for file_item in tqdm(self.file_list, desc=f'Caching latents{" to disk" if to_disk else ""}'):
                ...
                # TODO: 这里我们粗暴的设置 latent_space_version 为flux1，后续拓展应当及时跟进
                file_item.latent_space_version = 'flux1'
                ...
                if os.path.exists(latent_path):
                    ...
                    try:
                        ...
                        latent = self.sd.encode(imgs).latent_dist.mode().squeeze(0)
                    except Exception as e:

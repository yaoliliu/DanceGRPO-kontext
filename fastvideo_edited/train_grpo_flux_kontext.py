...
from fastvideo_edited.utils.parallel_states import (
    initialize_sequence_parallel_state,
    destroy_sequence_parallel_group,
    get_sequence_parallel_state,
    nccl_info,
)
from fastvideo_edited.utils.communications_flux import sp_parallel_dataloader_wrapper
from fastvideo_edited.utils.validation import log_validation
import time
...
from fastvideo_edited.utils.dataset_utils import LengthGroupedSampler
import wandb
from accelerate.utils import set_seed
from tqdm.auto import tqdm
from fastvideo_edited.utils.fsdp_util import get_dit_fsdp_kwargs, apply_fsdp_checkpointing
from fastvideo_edited.utils.load import load_transformer
from diffusers.optimization import get_scheduler
from diffusers.utils import check_min_version
from fastvideo_edited.dataset.latent_flux_rl_datasets import LatentDataset, latent_collate_function
import torch.distributed as dist
from torch.distributed.fsdp import FullyShardedDataParallel as FSDP
from fastvideo_edited.utils.checkpoint import (
    save_checkpoint,
    save_lora_checkpoint,
)
from fastvideo_edited.utils.logging_ import main_print
import cv2
...
import yaml
from toolkit.config_modules import preprocess_dataset_raw_config, DatasetConfig
...
def main(args):
    ...
    t5_tokenizer = T5Tokenizer.from_pretrained(args.pretrained_model_name_or_path, subfolder="tokenizer_2")
    clip = CLIPTextModel.from_pretrained(args.pretrained_model_name_or_path, subfolder="text_encoder", torch_dtype = torch.bfloat16,).to(device)
    t5 = T5EncoderModel.from_pretrained(args.pretrained_model_name_or_path, subfolder="text_encoder_2", torch_dtype = torch.bfloat16,).to(device)
    ...
    datasets = []
    dataset_configs = preprocess_dataset_raw_config(dataset_configs.get("datasets", []))
    if dataset_configs is not None and len(dataset_configs) > 0:
        for raw_dataset in dataset_configs:
            dataset = DatasetConfig(**raw_dataset)
            is_caching = True
            if not is_caching:
                is_latents_cached = False
            if dataset.is_reg:
                # TODO: support reg datasets
    ...
    train_dataloader, sampler, num_samples = get_dataloader_from_datasets(datasets, args.train_batch_size, vae, world_size=world_size, rank=rank, seed=args.sampler_seed)
    ...
    main_print(f"  Num examples = {num_samples}")
